...
> 历史说明：本文记录 2026-08-18 的早期三阶段方案调研，用于保留选型依据。当前版本采用“Playwright MCP 快速验证 + Cursor Browser 验证 + 人工处理”的两阶段自动化设计；本文中的三阶段状态模型不再是当前契约。当前决策以 [MVP 方案](../方案.md) 和[两阶段测试设计](../two-stage-design.md)为准。
