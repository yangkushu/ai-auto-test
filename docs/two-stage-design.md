...
> 本文记录下一版本的已确认设计，不表示 `0.2.0-dev.7` 已实现这些能力。当前可用实现仍是 Cursor 原生 Browser 单阶段测试，见[当前 MVP 方案](方案.md)和[架构设计](architecture.md)。
...
本设计实现时必须升级结果 Schema 并新建 run，不得向当前单阶段历史追加两阶段记录。实现前应先在 Cursor 中确认官方 Playwright MCP 可连接，并以相同的 10 条测试用例分别运行单阶段 Browser 与两阶段 `stage=auto`，对比：
