...
- Agent 新生成的 `summary.md`、`测试报告.md`、`message`、`reason`、`observed`、诊断和人工处理说明统一使用中文；技术术语首次可使用“中文（English）”。
...
读取本 Skill 同目录的 `VERSION` 作为 `skill_version`，使用 `schema_version=4`。开始前回显并在 `summary.md` 中记录最终生效的运行模式、Skill 版本和结果 Schema 版本。
...
完成跨文件检查并确定自检结论后、追加 `self_check_finished` 前，生成 `测试报告.md`。该报告是必需交付物：生成或保存失败必须使 `self_check=failed`、`run_valid=false`。报告必须全中文表达，只有 ID、URL、版本、机器原因码、原始页面/用例文案和浏览器原始错误可保持原样；不得写入凭据或验证码。报告只保留必要信息，不复制完整事件日志或逐步执行历史，至少包含：

1. 报告结论：测试范围、执行数量、中文状态统计、运行记录是否有效，以及“全部通过”“存在不通过项”“存在测试受阻/无法判断项”或“运行记录无效”之一；
2. 测试范围：目标环境、起止时间、运行模式、用例来源和 Browser-only 未覆盖边界；
3. 用例结果明细：用例 ID、标题、中文结论、关键可见结果、是否需要人工处理、证据相对路径；
4. 问题与人工处理：所有 `failed`、`blocked`、`inconclusive` 的事实、原因码、建议；Preflight 失败使用“事实、推断、建议”；
5. 测试数据影响：写操作的可见资源标识、清理状态和待人工清理项；无写操作时明确写“无”；
6. 证据与限制：Preflight 结论、截图索引和可用时的 SHA-256；正常模式的通过用例无截图必须明确说明为既定策略，不能描述为证据缺失。

完成前重新读取五个结果文件和证据目录，逐项检查：
...
- summary 状态计数与状态表一致；
- `测试报告.md` 存在、使用中文、包含结论、范围、用例明细、人工处理、测试数据影响和证据/限制；其统计与最新状态一致；
